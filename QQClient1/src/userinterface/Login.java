...
public class Login extends JFrame implements ActionListener
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			Examine yz=new Examine();
			User u=new User();//保存下输入的登录信息
			u.setUserId(jp2_jtf.getText().trim());
			u.setPasswd(new String(jp2_jpf.getPassword()));
			if(yz.checkUser(u))
			{
				try {
					FriendInterface friendlist=new FriendInterface(u.getUserId());//产生好友列表
					Friend.addlist(u.getUserId(),friendlist);
					ObjectOutputStream oos=new ObjectOutputStream(Link.getLink(u.getUserId()).getSocket().getOutputStream());
